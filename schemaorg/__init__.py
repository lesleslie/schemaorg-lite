...
from schemaorg.utils import print_json
from schemaorg.utils import read_file
...
class Schema(object):
    ...
    def _set_base(self, base):
        ...
        if not re.search("^http", base):
            logger.exit("%s must be a valid URL starting with http or https.")

        logger.info("Specification base set to %s" % base)
        self.base = base
    ...
    def _set_version(self, version):
        ...
        if not version in available:
            logger.warning("Version %s is not found in the data folder." % version)
            version = available[-1]
    ...
    def _load_custom_props(self, key="mapping", field="property"):
        ...
        logger.info("%s: found %s properties" % (self.type, len(self._properties)))
    ...
    def _load_type(self, schema_type):
        ...
        if schema_type not in typs:
            logger.error("%s is not a valid type!" % schema_type)
            self.print_similar_types(schema_type)
            sys.exit(1)

        self._set_type(schema_type)
        logger.info("Found %s" % self.url)
